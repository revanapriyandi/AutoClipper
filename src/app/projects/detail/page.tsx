...
function ProjectDetailsContent() {
    ...
    try {
      ...
      if (res.success && res.captions) {
        ...
        setCaptionVariants(prev => ({
          ...
          [index]: [
            `🔥 ${hook}... #shorts #viral`,
            `💡 Did you know? ${hook}... #learnontiktok`,
            `👀 ${hook}... #trending #fyp`,
          ]
        }));
      }
  ...
  const handleRender = useCallback(async (clip: ScoredCandidate, index: number) => {
    ...
    setStatus(`⚙️ Queuing render for Clip #${index + 1}...`);
      ...
      const interval = setInterval(async () => {
        ...
        if (jobRes?.success && jobRes.job) {
          ...
          if (job.status === "COMPLETED") {
            ...
            setStatus(`✅ Clip #${index + 1} rendered! Click 'Preview' or 'Open File' to view.`);
  ...
  const handlePostSubmit = async () => {
    ...
    if (res.success) setStatus(`📅 Clip scheduled for ${postPlatform} at ${combinedDateTime.toLocaleString()}!`);
